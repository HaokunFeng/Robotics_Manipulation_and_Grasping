    ...
    def execute(self):
        ...
        for action in self.actions:
            if action.action == ActionSaver.MOVE:
                ...
                ar = robot_api.pose2matrix(action.arPose)

                ar2wrist = robot_api.pose2transform(action.arPose, action.wristPose, True)
                # Save the ar marker's id and load it
                wrist = np.dot(robot_api.pose2matrix(filter(lambda x: x.id == action.arId, self.markers)[0].pose.pose), ar2wrist)
                ...
                pose_stamped.pose = robot_api.matrix2pose(wrist)
